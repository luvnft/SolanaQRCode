          ...
          {cart.length === 0 ? (
            ...
            cart.map((item, index) => (
              <div key={index} className="cart-item">
                <p>{item.name} - ${item.price} x {item.quantity} - {(convertToSOL(item.price * item.quantity)).toFixed(4)} SOL</p>
              </div>
            ))
          )}
          <h3>
            Total: $ {cart.reduce((acc, item) => acc + item.price * item.quantity, 0).toFixed(2)} - 
            {(convertToSOL(cart.reduce((acc, item) => acc + item.price * item.quantity, 0))).toFixed(4)} SOL
          </h3>
          {cart.length > 0 && (
